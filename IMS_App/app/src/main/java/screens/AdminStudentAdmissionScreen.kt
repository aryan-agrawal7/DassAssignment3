...
@Composable
fun AdminStudentAdmissionScreen(navController: NavController) {
    Scaffold(
        ...
        topBar = {
            Surface(color = BackgroundDark, modifier = Modifier.fillMaxWidth()) {
                Row(modifier = Modifier.padding(horizontal = 20.dp, vertical = 16.dp), verticalAlignment = Alignment.CenterVertically) {
                    ...
                    Text("Student Admission", color = TextWhite, fontSize = 16.sp, fontWeight = FontWeight.Bold)
                }
            }
        }
    ) { paddingValues ->
            ...
            Column(horizontalAlignment = Alignment.CenterHorizontally, modifier = Modifier.padding(horizontal = 24.dp)) {
                Surface(shape = CircleShape, color = CardDark, modifier = Modifier.size(112.dp)) {
                    Box(contentAlignment = Alignment.Center) {
                        Icon(Icons.Default.School, contentDescription = null, tint = TealAccent, modifier = Modifier.size(42.dp))
                    }
                }
                ...
                Text("Admission Module", color = TextWhite, fontSize = 28.sp, fontWeight = FontWeight.Bold)
